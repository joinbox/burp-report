(function() {
    ...
    const TaskScheduler = require('ee-taskscheduler');
        ...
        startServer() {
            ...
            this.loadCA().then((ca) => {
                ...
                return this.loadKeyPair('server').then((server) => {
                    ...
                    this.scheduler.schedule(['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'], this.config.reports.mail.times, 'mail', 3600);
                    this.scheduler.on('task', (group, id, next) => {
                        this.sendReport().then(next).catch((err) => {
                            this.warn(`Sending the report fialed: ${err.message}`);
                            next();
                        });
                    });
                });
        ...
        collectCAInfo() {
            ...
            return this.collectInfo().then((data) => {
                ...
                this.config.mail = {
                      transport: 'sendmail'
                    , times: ['08:00']
                    , from: 'backup-report@'+this.config.host
